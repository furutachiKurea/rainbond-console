...
class AppService(object):
    ...
    def create_kubeblocks_component(self,
        ...
        Args:
            ...
            tenant: 租户对象
            user: 用户对象
            service_cname (str): 组件显示名称
            k8s_component_name (str): Kubernetes 组件名称
            arch (str): 架构类型，默认 amd64

        Returns:
            tuple: (状态码, 消息, 组件对象)
                  - 200, "创建成功", service 表示成功
                  - 4xx, 错误消息, None 表示失败
        """
        # 参数验证：检查组件名称是否合规
        service_cname = service_cname.rstrip().lstrip()
    ...
    def __init_kubeblocks_component(self, region):
        ...
        tenant_service.service_origin = "assistant"   # 标识为平台创建
        tenant_service.create_status = "creating"
